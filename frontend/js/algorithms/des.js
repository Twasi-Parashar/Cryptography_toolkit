...
export async function send(payload, type) {
    ...
    // Validate inputs
    if (!key) {
        document.getElementById('result-text').textContent = '⚠️ Please enter the DES key.';
        return;
    }
    if (key.length !== 16) {
        document.getElementById('result-text').textContent =
            '⚠️ DES key must be 16 hexadecimal characters.';
        return;
    }

    const body = { text, key };
    const url = `http://localhost:4000/api/des/${type}`; // Matches backend route structure
