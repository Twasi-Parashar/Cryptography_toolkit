...
export async function send(payload, type) {
    ...
    // Validate inputs
    if (!key) {
        document.getElementById('result-text').textContent = '⚠️ Please enter the number of rails.';
        return;
    }

    const body = { text, key };
    const url = `http://localhost:4000/api/railfence/${type}`; // Matches backend route structure
